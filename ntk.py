...
def normalize(data):
    data_norm = data.reshape(-1, 3072.)
    data_norm = data_norm - np.sum(data_norm, axis=1, keepdims=True)/(32.*32.*3.)
    data_norm = data_norm/np.linalg.norm(data_norm, axis=1, keepdims=True)
    return data_norm

def make_MNIST_shifted(data, n, img_size, corner = (0,0)):
  new_data = []
  for idx in range(n):
    new_img = numpy.zeros((img_size[0], 2*img_size[1], img_size[2]))
...
n_samples = 10 # training samples per class
...
# generate shifted MNIST dataset
  ...
  x_test = numpy.concatenate([planes_x[n_samples:n_samples+n_test], ships_x[n_samples:n_samples+n_test]])
  x_test = numpy.array([resize(image, img_size) for image in x_test])
  x_test = make_MNIST_shifted(x_test, 2*n_test, img_size, (0,16))
...
def ConvNetLinear(depth, D, pool=False, linear=True):
  layers = []
  if linear:
    ...
    w = 2**0.5
  for d in range(depth):
    layers += [nt.stax.Conv(1, (7, 7), strides=(1, 1), W_std=w, padding='SAME')]
    if not linear:
      layers += [nt.stax.Relu()]
  if pool:
    layers += [nt.stax.GlobalAvgPool()]
  else:
    layers += [nt.stax.Flatten(), nt.stax.Dense(1, 1.0)]
  return nt.stax.serial(*layers)

def train(kernel_fn, x_train, y_train, x_test):
  ...
  return loss_d, acc_d, loss_t, acc_t, norm
...
for depth in depths:
  ...
  loss_d, acc_d, loss_t, acc_t, norm  = train(FCN, x_train, y_train, x_test)
  print(f'depth: {depth}, acc: {acc_d}, loss: {loss_d}, train_acc: {acc_t}, train_loss: {loss_t}, norm: {norm}')
...
for depth in depths:
  ...
  loss_d, acc_d, loss_t, acc_t, norm = train(CNN_pool, x_train, y_train, x_test)
  print(f'[{depth}, {acc_d}, {loss_d}, {acc_t}, {loss_t}, {norm}],')